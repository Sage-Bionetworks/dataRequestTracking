...
cols = [
    Column(name="synapse_id", columnType="ENTITYID"),
...
schema = syn.store(
    synapseclient.table.Schema(
        name="Data Request Tracking Table", columns=cols, parent="syn26243167"
    )
)
...
schema = syn.store(
    synapseclient.table.Schema(
        name="Data Request changeLogs Table", columns=cols, parent="syn26243167"
    )
)
...
schema = syn.store(
    synapseclient.table.Schema(
        name="Data Request changeLogs (longVersion)", columns=cols, parent="syn26133760"
    )
)
table = syn.store(Table(schema, logs))
...
schema = syn.store(
    synapseclient.table.Schema(
        name="Jira_changeLogs", columns=cols, parent="syn26133760"
    )
)
...
schema = syn.store(
    synapseclient.table.Schema(
        name="Jira_changeLogs_longVersion", columns=cols, parent="syn26133760"
    )
)
table = syn.store(Table(schema, logs.drop(columns=["index"])))
...
schema = syn.store(
    synapseclient.table.Schema(
        name="1kD Team Members", columns=cols, parent="syn26133760"
    )
)
