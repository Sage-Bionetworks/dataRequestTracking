...
def create_issue(auth, submission):
   ...
   summary = f"1kD Access Request {submission['submission_id']}: {submission['folder_name']}/ Requester: {submission['submitter']} ({submission['team_name']})"
...
def main():
   ...
   for submission_id in requests['submission_id'].unique():
      if submission_id not in np.append(logs['submission_id'].unique(), ['5103','5104','6163','5138','5165','6357','5984','6356']):
         ...
         submission = requests.loc[requests['submission_id'] == "5986", ].to_dict('records')[0]
         #pdb.set_trace()
